...
\point{Example:} what is the size or cardinality of $\Omega$ (i.e. the number of elements in it) for our recurrent example version A? I will denote the size by $|\Omega|$. Answer:$2^6$ because there are six bits each free to take two possible values. We conclude that the probability of any given outcome in version A is $1/64$. Note that 
...
\point{Exercise:} To get a feeling for what this means, consider $X = (X_1, X_2)$ as the value on two dice, and $Y$ as the sum. We want an estimator for $X_1$. Consider $\delta_1 = 6 \1[Y > 6] + \1[Y \le 6]$ and $\delta_2 = X_1$. Check $\delta_1 \in \sigma(Y)$ but $\delta_2 \notin \sigma(Y)$.
...
\subsection{Solutions for exercise set 3}
\begin{enumerate}
...
%	Since $\mathcal F = \sigma(\mathcal S)$, $\mathcal S \subseteq \mathcal F$ so
%	we trivially have $1 \Rightarrow 2$.
%	For the converse, define the following collection of sets
%	$\lambda = \{A \in {\mathcal F} : \P_1(A) = \P_2(A)\}$.
%	We first show that this collection is indeed a $\lambda$-system.
%	\begin{enumerate}
%		\item $\Omega \in \lambda$ since $\Omega \in \mathcal F$ ($\sigma$-algebra)
%		and $\P_1(\Omega) = 1 = \P_2(\Omega)$ (probability axiom).
%		\item Suppose $A, B \in \lambda$ and $A \subseteq B$ then
%		\begin{equation*}
%		\left.\begin{aligned}
%		B \setminus A = B \cap A^c
%		&\overset{(a)}{\in} \mathcal F
%		\\
%		\P_1(B \setminus A)
%		\overset{(b)}{=} \P_1(B) - \P_1(A)
%		&\overset{(c)}{=} \P_2(B) - \P_2(A)
%		\overset{(b)}{=} \P_2(B \setminus A)
...
%		Because
%		\begin{itemize}
%			\item (a) $\sigma$-algebras are closed under countable intersections and the complement operation
%			\item (b) property of probability and the fact that $A \subseteq B$
%			\item (c) by assumption $A,B \in \lambda$
%		\end{itemize}
%		\item Consider an infinite sequence of $A_i \in \lambda$ such that
%		$A_i \uparrow A$. $A \in \mathcal F$ because $\sigma$-algebras are closed under countable union and 
%		\begin{equation*}
%		\P_1(A) = \lim \P_1(A_i) = \lim \P_2(A_i) = \P_2(A)
%		\end{equation*}
%		by continuity of probability measures and because $A_i \in \lambda$.
%	\end{enumerate}
%	Therefore $\lambda$ satisfies the three conditions of $\lambda$-systems (it finally earns its name).
%	
%	By assumption $\mathcal S$ is a $\pi$-system because it is closed under intersection, therefore if $\P_1(A) = \P_2(A),\ \forall A \in \mathcal S$
%	then $\mathcal S \subset \lambda$ and, by the $\pi$-$\lambda$ theorem,
%	$\mathcal F \subset \lambda$, which completes the proof.
...
\point{Fubini:} same statement as Tonelli, but instead of requiring $g \ge 0$, we ask that $\int |g| \ud(\mu_1 \times \mu_2) < \infty$. How to prove this finite absolute value condition? By first applying Tonelli on $|g| \ge 0$!
...
Now let us integrate both sides and use Fubini:
...
% exchange derivative and integrals

% more factoids about integrals
...
\point{Partial converse:} back to the relation between convergence in probability and convergence in mean. For $p=1$, we have $X_n \pcv X$ implies $X_n \lpcv X$ if and only if they are \emph{uniformly integrable}, defined as: for all $\epsilon > 0$, there is $K > 0$ such that $\sup \E[|X| \1(|X| \ge K)] \le \epsilon$. 
