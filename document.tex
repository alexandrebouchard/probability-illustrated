...
\point{Proposition:} Suppose $\P_1$ and $\P_2$ are probability measures on $\events = \sigma(\generators)$, where $\generators$ is closed under finite intersection. Then the following are equivalent:
\begin{enumerate}
...
\point{For computations:} it can be easier to compute the derivatives of $g$ rather than $g^{-1}$, and thankfully using properties of Jacobian matrices $J$,
...
\point{Proof:} let $C_\epsilon = (|X_n - X| > \epsilon\io)$. By monotonicity of the $C_\epsilon$, we have $\P(C_\epsilon) = 0$ for all $\epsilon > 0$ if and only if $\P(C_{1/n}) = 0$ for all $n \in \N$.  Note 
\begin{eqnarray*}
...
\begin{enumerate}
  \item we have convergence of row sums of probabilities to $\lambda$: $\lim_{n\to\infty} \sum_{j=1}^n p_{n,j} = \lambda$,
...
\begin{itemize}
  ...
  \item What is the approximate distribution of the number of mutations in the first half of the genome? Last quarter? Are those independent?
\end{itemize}
\begin{center}
...
\point{Main result:} these constraints can be used to find the numerical value of $h(x)$. More precisely, $h(x)$ is the minimum function satisfying the above three conditions, i.e., if $h'$ satisfies the three conditions above, then $h(x) \le h'(x)$ for all $x \in \states$. 
...
\begin{enumerate}
  ...
  \item It is therefore enough to show that the two chain meet ``quickly,'' formally that $\P(T > n) \to 0$. This is where aperiodicity and irreducibility are used! What happens if not irreducible? If not aperiodic? Consider counter example at the beginning of the section to understand the importance of irreducibility and aperiodicity.
  ...
  \begin{align*}
  	...
  	&= \sum_{m=1}^n \sum_x \P(T=m, Y_m = x) \P(Y_n = y | T=m, Y_m = x) \text{ (marginals match)} \\
  	&= \P(Y_n = y, T \le n)\\
  \end{align*}
  which implies
  \begin{align*}
