...
\point{Question 4:} we start with the weaker result, convergence in probability, which is more intuitive; i.e. a simple modification of our argument for WLLN \begin{enumerate}
	\item First, assume without loss of generality that $\mu = 0$ (otherwise, set $X'_i = X_i - \mu$). 
	\item Use Chebyshev:
	\begin{eqnarray*}
...
\point{Theorem:} if $M^N$ has no zero entries,\footnote{There are far more general conditions, but they need more setup. We will hopefully get to them soon.} there exists a distribution $\pi$ such that we have the following LLN:
