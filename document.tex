...
\point{Method 1: } One way to do this is to force $\delta$ to be a composition based on $X$ only: $\delta = f(X)$ for some $f$. 
...
\point{Equivalence of the 2 methods: } In an optional question in the assignment, you will show: $\sigma(\delta) \subset \sigma(X)$ if and only if $\delta = f(X)$ for some measurable $X$.

\point{Notation:} We use the shorthand $\delta \in \sigma(X)$ for $\sigma(\delta) \subset \sigma(X)$.
